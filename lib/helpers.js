...
// Calibrate moment to proper timezone
...
const getUserInfo = function(userId, db) {
  return new Promise((resolve, reject) => {
    if (userId) {
      ...
      const userInfoQuery = {
        text: `SELECT id, name, phone_number, email, admin FROM users WHERE id = $1`,
        values: [userId],
      };
...
const getPhoneNumber = (userId, db) => {
  ...
  default:
    return db.query(phoneNumberQuery, [userId])
      .then(data => data.rows[0].phone_number)
      .catch(err => err);
  }
};
...
// Generates query object from a cart object
